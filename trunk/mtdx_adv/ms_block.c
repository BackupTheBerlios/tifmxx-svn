...
struct ms_block_data {
	...
	struct mtdx_request       internal_req;
	int                       (*mrq_handler)(struct memstick_dev *card,
						 struct memstick_request **mrq);

	enum memstick_command     cmd;
	struct ms_param_register  cmd_param;
	struct ms_status_register cmd_status;
...
static const struct ms_register_addr ms_block_r_all_w_param = {
	.r_offset = 0,
	.r_length = sizeof(struct ms_register),
	.w_offset = offsetof(struct ms_register, param),
	.w_length = sizeof(struct ms_param_register)
};
...
static int ms_block_complete_req(struct memstick_dev *card,
				 struct memstick_request **mrq);
...
static int h_ms_block_read_status(struct memstick_dev *card,
				  struct memstick_request **mrq);
static int h_ms_block_write_extra(struct memstick_dev *card,
				  struct memstick_request **mrq);
static int h_ms_block_write_data(struct memstick_dev *card,
				 struct memstick_request **mrq);
static int h_ms_block_read_data(struct memstick_dev *card,
				struct memstick_request **mrq);
...
static int ms_block_trans_req(struct ms_block_data *msb)
{
	...
	ms_param_set_addr(&msb->cmd_param, msb->req_in->phy.b_addr);

	msb->dst_page = msb->cmd_param.page_address;
	msb->page_count = msb->req_in->length / msb->geo.page_size;

	dev_dbg(&msb->card->dev, "trans req %x, %x, %x:%x\n", msb->req_in->cmd,
	...
	case MTDX_CMD_READ:
		...
		if (msb->req_in->req_oob) {
			...
			else if (msb->caps & MEMSTICK_CAP_REG_QUIRK) {
				msb->cmd_flags &= ~MS_BLOCK_FLG_PAGE_INC;
				msb->cmd_param.cp = MEMSTICK_CP_PAGE;
			}
		}
			       ...
			       struct memstick_request **mrq)
{
	...
	*mrq = &card->current_mrq;
	...
	do {
		...
		if (msb->req_in) {
			dev_dbg(&card->dev, "submit request %x, %x, %x:%x\n",
				msb->req_in->cmd, msb->req_in->phy.b_addr,
				msb->req_in->phy.offset, msb->req_in->length);
			rc = ms_block_trans_req(msb);
			if (!rc)
				break;
			else {
				msb->req_dev->end_request(msb->req_dev,
							  msb->req_in, 0,
							  rc, 0);
				msb->req_in = NULL;
				msb->cmd_flags = 0;
	...
	dev_dbg(&card->dev, "4 dev %p, req %p\n", msb->req_dev, msb->req_in);
	...
	if (!(msb->cmd_flags & MS_BLOCK_FLG_WRITE)
	    && (msb->cmd_flags & MS_BLOCK_FLG_EXTRA)
	    && !(msb->caps & MEMSTICK_CAP_REG_QUIRK))
		memstick_init_req(*mrq, MS_TPC_SET_RW_REG_ADRS,
				   ...
				   int error)
{
	...
	if (msb->cmd_flags & MS_BLOCK_FLG_PAGE_INC) {
		...
		return ms_block_complete_req(card, mrq);
}
				     ...
				     struct memstick_request **mrq)
{
	...
	ms_block_reg_addr_set(card, (struct ms_register_addr *)((*mrq)->data));
	...
	if (!(msb->cmd_flags & MS_BLOCK_FLG_COPY)) {
		...
		msb->cmd_param.page_address = msb->dst_page;
	} else {
		msb->cmd_param.cp = MEMSTICK_CP_PAGE;
		msb->cmd_flags &= ~MS_BLOCK_FLG_DATA;
		...
		if (msb->cmd == MS_CMD_BLOCK_READ) {
			msb->cmd = MS_CMD_BLOCK_WRITE;
			msb->cmd_flags |= MS_BLOCK_FLG_WRITE;
			ms_param_set_addr(&msb->cmd_param,
					  msb->req_in->phy.b_addr);
			msb->cmd_param.page_address = msb->dst_page;
		} else {
			msb->cmd = MS_CMD_BLOCK_READ;
			msb->cmd_flags &= ~MS_BLOCK_FLG_WRITE;
	...
	memstick_init_req(*mrq, MS_TPC_WRITE_REG, &msb->cmd_param,
			  sizeof(msb->cmd_param));
	card->next_request = h_ms_block_write_param;

	return 0;
}

static int h_ms_block_read_extra_reg(struct memstick_dev *card,
				     struct memstick_request **mrq)
{
	struct ms_block_data *msb = memstick_get_drvdata(card);
	struct ms_register *ms_reg = (struct ms_register *)((*mrq)->data);
	...
	memmove(&ms_reg->extra_data, (*mrq)->data, sizeof(ms_reg->extra_data));
	memcpy(&ms_reg->status, &msb->cmd_status, sizeof(ms_reg->status));

	return h_ms_block_read_status(card, mrq);
}
				       ...
				       struct memstick_request **mrq)
{
	...
	card->next_request = h_ms_block_read_extra_reg;

	return 0;
}


static int h_ms_block_read_status_reg(struct memstick_dev *card,
				      struct memstick_request **mrq)
{
	struct ms_block_data *msb = memstick_get_drvdata(card);
	...
	memcpy(&msb->cmd_status, (*mrq)->data, sizeof(msb->cmd_status));
...
static void ms_block_set_status_reg(struct memstick_dev *card,
				    struct memstick_request **mrq)
{
	struct ms_block_data *msb = memstick_get_drvdata(card);

	if (!(msb->caps & MEMSTICK_CAP_REG_QUIRK)) {
		memstick_init_req(*mrq, MS_TPC_READ_REG, NULL,
		...
		card->next_request = h_ms_block_read_status_reg;
	}
}
				  ...
				  struct memstick_request **mrq)
{
	...
	case MS_CMD_BLOCK_READ:
		...
		if (msb->cmd_flags & MS_BLOCK_FLG_EXTRA)
			ms_block_set_status_reg(card, mrq);
	...
	case MS_CMD_BLOCK_WRITE:
		...
		if (msb->cmd_flags & MS_BLOCK_FLG_PAGE_INC) {
			...
			if (msb->t_count == msb->page_count)
				ms_block_complete_multi(card, mrq, 0);
				  ...
				  struct memstick_request **mrq)
{
	...
	if (status->interrupt & MEMSTICK_INT_ERR) {
		...
		if (!(msb->cmd_flags & MS_BLOCK_FLG_COPY))
			dev_info(&card->dev, "Correctable read error at "
				 "%x:%x\n", msb->req_in->phy.b_addr,
				 msb->dst_page);
		else {
			dev_info(&card->dev, "Correctable read error at "
	...
	if (msb->cmd_flags & MS_BLOCK_FLG_EXTRA) {
		...
		*extra = ((struct ms_register *)((*mrq)->data))->extra_data;
	...
	if (msb->cmd_flags & MS_BLOCK_FLG_DATA) {
	...
	} else if (msb->cmd_flags & MS_BLOCK_FLG_COPY) {
		msb->src_pos.offset++;
		...
		if (!ms_block_reg_addr_cmp(card, (struct ms_register_addr *)
						 ((*mrq)->data)))
			return h_ms_block_set_param_addr(card, mrq);
	} else
		BUG();
...
static int h_ms_block_data_get_int_r(struct memstick_dev *card,
				     struct memstick_request **mrq)
{
	unsigned char int_reg = (*mrq)->data[0];
	struct ms_block_data *msb = memstick_get_drvdata(card);

	dev_dbg(&card->dev, "data_get_int_r: %d, %x\n", (*mrq)->error, int_reg);
	...
	if (!msb->ced_signalled && !(int_reg & MEMSTICK_INT_BREQ)) {
		msb->breq_count++;
		if (msb->breq_count > BREQ_RETRIES)
			return ms_block_complete_multi(card, mrq, -EIO);
		else
...
static int h_ms_block_data_get_int_w(struct memstick_dev *card,
				     struct memstick_request **mrq)
{
	unsigned char int_reg = (*mrq)->data[0];
	struct ms_block_data *msb = memstick_get_drvdata(card);

	dev_dbg(&card->dev, "data_get_int_w: %d, %x\n", (*mrq)->error, int_reg);
	...
	if (msb->t_count == msb->page_count)
		...
		if (msb->cmd_flags & MS_BLOCK_FLG_OV) {
			extra->overwrite_flag = ~MEMSTICK_OVERWRITE_UDST;
			extra->management_flag = 0xff;
			extra->logical_address = 0xffff;
		}

		memstick_init_req(*mrq, MS_TPC_WRITE_REG, extra,
				  sizeof(*extra));
		...
		card->next_request = h_ms_block_write_data;
		(*mrq)->need_card_int = 1;
	} else
		BUG();
				...
				struct memstick_request **mrq)
{
	...
	if (!(msb->cmd_flags & MS_BLOCK_FLG_PAGE_INC)) {
		...
		if (msb->cmd_flags & MS_BLOCK_FLG_COPY) {
			msb->src_pos.offset++;
			mtdx_data_iter_dec(msb->req_in->req_data,
					   msb->geo.page_size);
		} else if ((msb->cmd_flags & MS_BLOCK_FLG_EXTRA)
			   && (!(msb->caps & MEMSTICK_CAP_REG_QUIRK)))
			memstick_init_req(*mrq, MS_TPC_SET_RW_REG_ADRS,
		...
		if ((msb->t_count == msb->page_count) || msb->ced_signalled)
			return ms_block_complete_multi(card, mrq, 0);

		memstick_init_req(*mrq, MS_TPC_GET_INT, NULL, 1);
		card->next_request = h_ms_block_data_get_int_r;

		if (msb->caps & MEMSTICK_CAP_AUTO_GET_INT) {
			(*mrq)->data[0] = (*mrq)->int_reg;
			return h_ms_block_data_get_int_r(card, mrq);
		}
	}
				 ...
				 struct memstick_request **mrq)
{
	...
	if (!(msb->cmd_flags & MS_BLOCK_FLG_PAGE_INC)) {
		...
		memstick_init_req(*mrq, MS_TPC_GET_INT, NULL, 1);
		card->next_request = h_ms_block_data_get_int_w;

		if (msb->caps & MEMSTICK_CAP_AUTO_GET_INT) {
			(*mrq)->data[0] = (*mrq)->int_reg;
			return h_ms_block_data_get_int_w(card, mrq);
		}
	}
				 ...
				 struct memstick_request **mrq)
{
	...
	return h_ms_block_req_init(card, mrq);
}
...
static int ms_block_switch_to_parallel(struct memstick_dev *card)
{
	...
	struct ms_param_register param = {
		.system = msb->cmd_param.system | MEMSTICK_SYS_PAM,
		.block_address = { 0, 0, 0},
	...
	msb->cmd_param.system = param.system;
	...
	if (rc) {
		...
		msb->cmd_param.system = MEMSTICK_SYS_BAMD;
				      ...
				      unsigned int start_addr)
{
	...
	msb->internal_req.logical = MTDX_INVALID_BLOCK;
	msb->internal_req.phy.b_addr = MTDX_INVALID_BLOCK;
	msb->internal_req.phy.offset = 0;
	msb->internal_req.length = sizeof(struct ms_block_boot_header);
	msb->internal_req.req_data = &r_data;
	msb->internal_req.req_oob = &r_oob;
	...
	for (b_cnt = start_addr; b_cnt < MS_BLOCK_MAX_BOOT_ADDR; ++b_cnt) {
		msb->internal_req.phy.b_addr = b_cnt;
		msb->req_dev = msb->mdev;
		msb->int_issue = 1;
		...
		if (msb->int_count != msb->internal_req.length) {
			dev_dbg(&card->dev, "got %x bytes, expected %x\n",
				msb->int_count, msb->internal_req.length);
			return MTDX_INVALID_BLOCK;
		}
				    ...
				    struct ms_block_boot_ref *b_ref)
{
	...
	msb->internal_req.cmd = MTDX_CMD_READ;
	msb->internal_req.logical = MTDX_INVALID_BLOCK;
	msb->internal_req.phy.b_addr = b_ref->phy_block;
	msb->internal_req.phy.offset = 0;
	msb->internal_req.length = 0;
	msb->internal_req.req_data = &r_data;
	msb->internal_req.req_oob = &r_oob;
	...
	msb->internal_req.length = b_ref->size;
...
static int ms_block_init_card(struct memstick_dev *card)
{
	...
	if (msb->caps & MEMSTICK_CAP_PAR4) {
		if (ms_block_switch_to_parallel(card))
			dev_err(&card->dev, "could not select media "
				"interface\n");
	}

	if (msb->cmd_param.system & MEMSTICK_SYS_PAM)
				      ...
				      struct mtdx_dev *req_dev)
{
	...
	dev_dbg(&msb->card->dev, "get device %p\n", req_dev);
...
static void ms_block_remove(struct memstick_dev *card)
{
	...
	do {
		c_dev = mtdx_dev_queue_pop_front(&msb->c_queue);
		if (c_dev) {
			dev_dbg(&card->dev, "put device %p\n", c_dev);
